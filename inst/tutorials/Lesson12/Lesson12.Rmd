---
title: "Lesson 12: T-tests for group comparison"
...
##### פונקציה לבדיקה
exercises_df = data.frame(exercises = c("critical_t_ex",
                                        "one_samp_t_output_question", 
                                        "two_sample_t_q", 
                                        "paired_t_q", 
                                        "outlier_t_ex", 
                                        "final_q_5", 
...
hebrew = c("התפלגות: תרגיל", 
              "מבחן למדגם בודד: שאלה",
              "מבחן למדגמים בלתי תלויים: שאלה", 
              "מבחן למדגמים מזווגים: שאלה",
              "תצפיות קיצוניות: תרגיל", 
              "תרגיל מסכם - שאלה 1: ערך מחושב",
              "תרגיל מסכם - שאלה 1: מסקנה",
              "תרגיל מסכם - שאלה 2: רווח סמך 1",
              "תרגיל מסכם - שאלה 2: רווח סמך 2",
              "תרגיל מסכם - שאלה 3: ערך מחושב",
              "תרגיל מסכם - שאלה 3: מובהקות",
              "תרגיל מסכם - שאלה 3: מסקנה" 
           # ,"הגשת קוד התרגיל המסכם"
           )
  )
...
את צורת ההתפלגות ייצרנו באמצעות הפונקציה `dt()` אשר מקבלת את x ואת דרגות החופש. 
...
ועכשיו יש לנו את כל מה שדרוש כדי לבצע את המבחן. אנחנו יכולים לבצע מבחן ידנית על ידי השוואה של ערך שהתקבל במדגם מסוים להתפלגות הt המתאימה לו על פי דרגות החופש.   

נוכל לעשות זאת על ידי שימוש בפונקציה `pt()`.   
פונקציה זאת בעצם מבצעת את הפעולת ההפוכה לqt().  
 נוכל להכניס אליה את הערך שחושב במדגם ולקבל מה ההסתברות שלו תחת התפלגות הt. או במילים אחרות, אם נכניס את ערך הt המחושב ונבין מה ההסתברות לקבל אותו תחת ההתפלגות בעצם נוכל לחשב את ה`p-value`. 

נמשיך עם הדוגמה הקודמת שלנו ונוסיף לה ערך מחושב ממדגם היפותטי:
...
ggplot(data.frame(x, y), aes(x, y)) +
  geom_line(color = "blue", linewidth = 1) +  
  # נוסיף את הערך הקריטי להתפלגות באמצעות קו אונכי
  geom_vline(xintercept = samp_value, color = "green", linetype = "dashed", linewidth = 1) +  # עיצוב
  labs(
    title = "t-Distribution with Critical Values (df = 10)",
    x = "t-value",
    y = "Density"
...
`mu` - ממוצע ההשוואה עבור מבחן למדגם בודד.   
...
תזכורת - בגרסה המוכרת שלו, מציגים למשתתפים שמות של צבעים הצבועים בצבע. הצבע יכול להיות `תואם` למילה הכתובה או `לא תואם`. בתנאי התואם קל יותר למשתתפים לעבד את המילה ואילו בתנאי הלא תואם צריך לגייס שליטה קוגניטיבית ולכן זמן התגובה של המשתתפים לרוב גדול יותר בצורה משמעותית בתנאי הלא תואם. 
...
לצורך הדוגמה נרצה לבחון את ההשערה שזמן התגובה במדגם הנוכחי שונה מ2.  
...
question("איזו משורות הקוד הבאות יוכלו לעזור להבין האם ניתן לדחות את השערת האפס",
...
איור זה מציג את הממוצע (גובה העמודה הכחולה) של זמני התגובה במדגם. הממוצע המוכר מהספרות מסומן על ידי הקו האדום.
...
מערך הtהקיצוני וגם מערך הp ניתן להסיק שקיבלנו תוצאה מובהקת. אבל יש כמה נקודות  שכדאי לשים לב אליהן בפלט.   
...
הכנסה של ערך בודד אחד שאיננו מתאים להתפלגות זמני התגובה הביאה לכך שלא יכולנו לדחות את השערת האפס, עבור מדגם שאנחנו יודעים שכן קיים בו אפקט.  
...
נחזור לדוגמה שהתחלנו איתה, החוקר המקומי החליט שכדאי שהוא ישחזר את הממצא שלו עם מדגם דוברי העברית ודגם דגימה נוספת של משתתפים מקומיים. אם שני המדגמים היו אמינים ואמדו בצורה מדוייקת את זמן התגובה של המשתתפים, לא נצפה שתהיה תוצאה שונה באופן חריג במדגם השני. הנתונים טעונים ללומדה בתוך טבלה שנקראית outlier_data.
 שימו לב, בטבלה זאת מאוכסנות התוצאות של שני מדגמים. למרות ששני המדגמים מאוכסנים באותה טבלה הם לא מזווגים, אלא בלתי תלויים ויש להתייחס לכל עמודה כמדגם נפרד.  
...
# קיבלנו הבדל מובהק
# נציג את הנתונים של המדגם הראשון ונראה איך זה נראה
box_plot_1 <- ggplot(outlier_data,  aes(y = sample1)) + geom_boxplot()
box_plot_1
# והשני
box_plot_2 <- ggplot(outlier_data,  aes(y = sample2)) + geom_boxplot()
box_plot_2
...
box_plot_1 + ylim(c(1,3))
# והמדגם השני
box_plot_2 + ylim(c(1,3))
...
```{r outlier_t_ex, exercise=TRUE, exercise.eval = FALSE,exercise.reveal_solution=FALSE}
# עריכת הנתונים
...
## תרגיל מסכם
ברור לנו שלאורך כל הלומדה רק חיכיתם לעבוד עם נתונים אמיתיים והנה הגיע הרגע. נשתמש באותו מאגר נתונים בו השתמשתם לתרגיל המסכם בסמסטר הקודם על מטלת הסטרופ.  
כדי להקל עליכם הכנו לכם גרסה מסודרת ומסוכמת של הנתונים.  
הורידו את הקובץ הנתונים המסוכם עבור שיעור זה מהמודל וטענו את מבנה הנתונים לRstudio.  

תזכורת:
הקובץ מכיל כבר נתונים מסוננים ומסוכמים לנוחיותכם. 
בקובץ זה כל שורה מתארת את תוצאותיו המסוכמות של משתתף אחד.  
...
לעמודות המכילות את זמני התגובה בתנאי התואם והלא תואם קיימות מספר גרסאות. הגרסה הראשונה מכילה את זמן התגובה של כל הצעדים התואמים והלא תואמים בנוסף להן קיימות עמודות בהן זמני התגובה מחושבים בנפרד למילים בצבע אדום ולא אדום. עמודות אלה רלוונטיות לשאלה 3.  

`שאלה 1`  
נתחיל בבחינה של אפקט הסטרופ, נרצה לבחון האם זמני התגובה בתנאי הלא תואם ארוכים מזמני התגובה בתנאי התואם.  
בצעו את מבחן הt המתאים כדי לענות על שאלה זאת. הקפידו להשתמש בסוג המבחן הנכון עם ההשערה המתאימה לשאלת המחקר.  
לאחר ביצוע המבחן ענו על השאלות הבאות והציגו גרף קופסא אשר מציג את הפרשי הממוצעים.
לא זוכרים איך לייצר איור כזה? חזרו לחלק על מדגמים מזווגים.  

הכניסו את הערכים הבאים מתוך תוצאות המבחן מעוגל לשתי ספרות לאחר הנקודה העשרונית:
...
question_text("t value",
  ...
  answer("51.47", correct = TRUE),
  answer("15.68", correct = FALSE, message = "האם ערכתם מבחן למדגמים מזווגים?"),
  allow_retry = TRUE,
    
  correct="כל הכבוד! קיבלתם ערך משמעותי ", 
  incorrect = "וודאו שהשתמשתם בבעמודות הנכונות, שביצעתם מבחן מזווג ונסו שוב."
)
...
question_text("מהו הגבול התחתון של רווח הסמך (עם שתי ספרות לאחר הנקודה העשרונית)?",
...
question_text("מהו הגבול העליון של רווח הסמך (עם שתי ספרות לאחר הנקודה העשרונית)?",
  answer("48.20", correct = TRUE),
  answer("48.27", correct = TRUE),
  answer("-43.60", correct = TRUE),
  answer("-43.67", correct = TRUE),
...
בדקו את ההשערה המתוארת על ידי השוואה של הפרשי זמני התגובה (אפקט סטרופ) בצעדים שהוצגו באדום לכאלה שלא. השתמשו בהשערה החד זנבית המתאימה. 

צרו שתי עמודות חדשות האומדות את אפקט הסטרופ. בצעו זאת פעמיים,  אחת לצעדים בצבע אדום ואחת לצעדים בשאר הצבעים.  

שימו לב! ההחסרה צריכה להיות של הצעדים הלא תואמים פחות התואמים! הכיווניות חשובה!  

בצעו מבחן t על שתי העמודות שהתקבלו, תחת ההשערה שבצבע האדום האפקט יהיה קטן יותר (שימו לב לסדר ההכנס למבחן).  

הכניסו את הערכים הבאים מתוך תוצאות המבחן מעוגלים שתי ספרות לאחר הנקודה העשרונית. במידה ומופיע לכם מספר בפורמט מקוצר הכניסו אותו בפורמט זה:  
...
question_text(
  ...
  answer("-4.43", correct = TRUE),
  answer("-4.44", correct = TRUE),
  answer("-3.96", correct = FALSE, message= "האם ערכתם מבחן למדגמים מזווגים?"),
...
question_text(
  ...
  answer("0.00", correct = FALSE, message= "הכניסו את הערך בפורמט מקוצר"),
  answer("1", correct = FALSE, message = "שימו לב לכיוון ההשערה"),
  answer("9.54e-06", correct = FALSE, message = "האם השתמשתם במבחן מזווג?"),
  allow_retry = TRUE,
  correct="נכון מאוד!", 
...
question("מה המסקנה שניתן להסיק על סמך מבחן זה בלבד?",
