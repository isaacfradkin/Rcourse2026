...

{
  ...
  exercises_map <- c(
  "loops_ex1"     =  "יצירת לולא: תרגיל ראשון",
  "loops_ex2"     =  "יצירת לולאה: תרגיל שני",
  "loops_q1"      =  "לולאות: שאלה 1", 
  "loops_q2"      =  "לולאות: שאלה 2", 
  "loops_ex3"     =  "לולאות: חישוב ממוצע חברותיות",
  "loops_q3"      =  "לולאות: סימולציה",
  "functions_q1"  =  "פונקציות: שאלה 1", 
  "functions_exercise" =  "פונקציות: כתיבת פונקציה",
  "functions_q2"  =  "פונקציות: שאלה 2", 
  "no_return"     =  "פונקציות: תיקון פונקציה",
  "Q_final_NAs"    =  "תרגיל מסכם: כמות ערכים שסוננו", 
  "Q_final_A_shape"   = "תרגיל מסכם: צורת התפלגות A",
  "Q_final_E_shape"   = "תרגיל מסכם: צורת התפלגות E"
)
...
הניתוח ישמש אותנו גם כחזרה על החומר מסמסטר א'.
אנחנו לא מצפים שתזכרו את הפונקציות שנלמדו בעל פה, אלא שתחזרו לסיכומים וללומדות כדי להזכר בחומר.



יבאו את קובץ הנתונים "big_5_order_of_birth.csv" לסביבת העבודה שלכם בRstudio.
 

הנתונים בקובץ זה מתארים את ציוניהם של נבדקים ב5 מימדי אישיות מקובלים (Big 5: E, A, O, C, N), בנוסף לכמות הילדים שהיו להוריהם (birthn) ולמיקום שלהם בסדר הילודה (birthpos: 1 מתאר אח בכור, 2 מתאר אח שני וכו').

בתרגיל זה לא נתייחס לסדר הילודה או לגודל המשפחה, אלא רק למדדי האישיות.
...
צרו פונקציה בשם `clear outliers` הממסננת תצפיות קיצוניות.

הפונקציה תקבל כקלט וקטור מספרי ותחזיר וקטור מספרי באותו האורך בו הערכים הקיצוניים הוחלפו בערכים חסרים.

ערכים קיצוניים יוגדרו כערכים הגדולים או קטנים ממוצע הוקטור ביותר מ2.5 סטיות תקן.


כדי לבחון את תקינות הפונקציה - השתמשו בה על וקטור בו אתם יודעים מה מה הם הערכים הקיצוניים. למשל - בוקטור `c(1:10,1000)`כל הערכים אמורים להיות תקינים, חוץ מהערך האחרון.


אחרי שווידאתם את תקינות הפונקציה - היעזרו בה כדי לייצר טבלה חדשה בה הערכים הקיצוניים בכל אחד מחמשת המדדים הוחלף בערך חסר.

תוכלו להיעזר לשם כך בפונקציה `mutate`, בה תערכו את כל אחת מ5 עמודות המדדים בנפרד, ותסננו ממנה את הערכים הקיצוניים בעזרת הפונקציה שיצרתם.

```{r, eval = F}

big_5_order_of_birth =  big_5_order_of_birth %>%
  mutate(E = clear_outliers(E),
         A = clear_outliers(A),
         O = clear_outliers(O),
         C = clear_outliers(C),
         N = clear_outliers(N))
...
היעזרו בפונקציה `pivot_longer` שנלמדה בסמסטר הקודם (יחידה 5) כדי לקבץ את חמשת עמודות מדדי הbig 5 יחד.
  
 הטבלה שקיבלתם אמורה להכיל את העמודות הבאות:
...
כאשר השמות של שתי העמודות האחרונות יכולים להיות שונים, בהתאם לשמות שבחרתם לתת להם. 


```{r Q_final_NAs, echo = FALSE}

question_text(
  "<div style='direction: rtl;'>
  בדקו - כמה תצפיות סוננו מכל המדדים יחד?
  </div>",
...
**3)** יצירת היסטוגרמה לכל מדד

השתמשו בלולאה כדי לייצר היסטוגרמה המציגה את התפלגות המדד עבור כל אחד מחמשת המדדים.

בכל סבב של הלולאה היא תסנן את טבלת הנתונים כך שתכלול רק מדד מסוג אחד (באמצעות פונקציית `filter` שנלמדה ביחידה 5) ואז תייצר ותציג היסטוגרמה עבור הנתונים שנשארו בטבלה (באמצעות ggplot, יחידה 6).

שימו לב להוסיף לתרשימים כותרת המציינת את שם המדד, כדי שתדעו לייחס כל תרשים למדד המתאים.

כשתריצו את הלולאה יווצרו 5 תרשימים. תוכלו לעבור ביניהם באמצעות החצים שבפינה השמאלית עליונה של חלונית התרשימים (Plots) שבצד הימני התחתון של Rstudio (יחד עם Files ומספר חלוניות נוספות). 


```{r Q_final_A_shape, echo = FALSE}

question(
  "<div style='direction: rtl;'>
מה צורת ההתפלגות של ציוני הנעימות (agreeableness - A)?
</div>",

  answer("נורמלית בקירוב", correct = F),
  answer("דו שכיחית", correct = F),
  answer("אסימטרית עם זנב ימינה", correct = F),
  answer("אסימטרית עם זנב שמאלה", correct = TRUE),
...
```{r Q_final_E_shape, echo = FALSE}

question(
  "<div style='direction: rtl;'>
מה צורת ההתפלגות של ציוני המוחצנות (Extraversion - E)?
</div>",

  answer("נרומלית בקירוב", correct = T),
  answer("דו שכיחית", correct = F),
  answer("אסימטרית עם זנב ימינה", correct = F),
  answer("אסימטרית עם זנב שמאלה", correct = F),
...
בנוסף להגשת קוד ההאש של יחידה זו, הגישו גם את קובץ הקוד ששימש אתכם לטובת התרגיל, דרך תיבת ההגשה הייעודית במודל.
