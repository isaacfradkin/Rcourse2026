...
{
exercises_df = data.frame(exercises = c("NHST_Q", "BF_to_prob_q","BF1_q", "one_sided_Q", "paired_Q1", "paired_Q2", "corr_one_sided_Q"),
                          hebrew = c("פירוש תוצאות מבחן t", "עדכון הסתברויות על סמך BF", "שעות שינה באוניברסיטה אמריקאית",
                                     "כמות ממוצעת של פרוסות עגבנייה",
                                     "עמדות האכלת חתולים - שאלה ראשונה", "עמדות האכלת חתולים - שאלה שנייה",
                                     "תרגיל מסכם"))
...
```{r prepare-cats}
set.seed(7)
cats = data.frame(date = c(paste0(1:30,"/4/2026"),paste0(1:31,"/5/2026")),
                  forum = round(rnorm(61,8,3)),
                  education = round(rnorm(61,7.9,3)))
```
...
הערך שמהווה את ברירת המחדל עבור r הוא $\sqrt2/2 = 0.707$, שכן ערך זה מניח גדלי אפקט בינוניים. בפועל, אנחנו נתאים את ערך הסקאלה להשערות שלנו לגבי האפקטים הצפויים, אך גם לכך נגיע בהמשך הלומדה.
...
במקרה שלנו - אם ייחסנו לשתי ההשערות הסתברות זהה להיות נכונות לפני שאספנו את הנתונים שלנו (50% - 50%), ההסתברות ש$H_1$ תהיה נכונה בהינתן הנתונים תהיה:
...
נוכל להמחיש זאת באמצעות חיבור של תוצאות הניתוח עבור מספר מבחני BF שונים באמצעות הפקודה `c()`. פקודה זו בדרך כלל משמשת ליצירת וקטורים, אך כשמשתמשים בה על אובייקטים המתקבלים מניתוחי BF היא מחברת אותם יחד לפלט אחד:
...
המבחן המתאים יהיה חד זנבי ולכן  מבחן הt שישמש אותנו לבחינת השערה כזו יראה כך:
...
בדקו באמצעות BF האם תוחלת כמות פרוסות העגבנייה קטנה מ5 באופן מובהק סטטיסטית.
השאירו את פרמטר הסקאלה בערך ברירת המחדל (`r=0.707`).
שימו לב שהפקודה nullInterval בה אנו משתמשים ליצירת השערת חד זנבית מנוסחת בסולם המתוקנן של גדלי האפקט, ולכן גם אם השערת האפס שלנו היא 5, השימוש בארגומנט זה זהה לדוגמא שלמעלה.
...
question_text(
  "<div style='direction: rtl;'>
 פי כמה יותר סביר שתוחלת כמות הפרוסות קטנה מ5, ביחס להשערת האפס לפיה התוחלת היא 5 (יש לעגל לשתי ספרות אחרי הנקודה)?
  </div>",
  answer("4.7716", correct = F,  message = "האם עיגלתם ל2 ספרות אחרי הנקדה העשרונית?"),
 answer("4.771",correct = F,  message = "האם עיגלתם ל2 ספרות אחרי הנקדה העשרונית?"),
 answer("4.772", correct = F,  message = "האם עיגלתם ל2 ספרות אחרי הנקדה העשרונית?"),
 answer("4.77", correct = T),
 answer("4.7", correct = F,  message = "האם עיגלתם ל2 ספרות אחרי הנקדה העשרונית?"),
 answer("4.8",correct = F,  message = "האם עיגלתם ל2 ספרות אחרי הנקדה העשרונית?"),
...
הטבלה `cats` מכילה פירוט על צריכת אוכל יומית בשני עמדות האכלה במיקומים שונים בקמפוס.
חשבו את ערך הBF המתאר את יחס הסיכויים בין ההשערה לפיה החתולים אוכלים יותר בפינת ההאכלה שליד הפורום (העמודה `forum`) מאשר בפינת ההאכלה בקרבת בית הספר לחינוך (`education`) לעומת ההשערה החלופית לפיה תוחלת צריכת האוכל זהה בין העמדות.
 
שימו לב לערוך את המבחן הסטטיסטי המתאים לתיאור הנתונים, גם מבחינת התלות בין המדגמים וגם מבחינת כיווניות ההשערה (חד / דו זנבית).
השאירו את פרמטר הסקאלה בערך ברירת המחדל (`r=0.707`).


```{r  cats_data, exercise=TRUE, exercise.eval = FALSE, exercise.setup = "prepare-cats"}
print(head(cats))
...
```{r paired_Q1, echo = FALSE}

question(
  "<div style='direction: rtl;'>
על פי ערך הBF שהתקבל - איזו השערה סבירה יותר?
  </div>",
  answer("ההשערה לפיה תוחלת צריכת האוכל גבוהה יותר בעמדה שליד הפורום"),
  answer("ההשערה לפיה תוחלת צריכת האוכל גבוהה יותר בעמדה שליד חינוך"),
  answer("ההשערה לפיה תוחלת צריכת האוכל שונה בין העמדות"),
  answer("ההשערה לפיה תוחלת צריכת האוכל זהה בין העמדות", correct = T),
  allow_retry = TRUE,
...
```{r paired_Q2, echo = FALSE}

question_text(
  "<div style='direction: rtl;'>
  פי כמה השערה זו סבירה יותר מאשר ההשערה אליה השוותם אותה?  (עגלו את ערך הBF לשני מקומות אחרי הנקודה העשרונית)
  </div>",
  answer("3.49236", correct = F,  message = "האם עיגלתם ל2 ספרות אחרי הנקדה העשרונית?"),
 answer("3.4924",correct = F,  message = "האם עיגלתם ל2 ספרות אחרי הנקדה העשרונית?"),
 answer("3.492", correct = F,  message = "האם עיגלתם ל2 ספרות אחרי הנקדה העשרונית?"),
 answer("3.5", correct = F,  message = "האם עיגלתם ל2 ספרות אחרי הנקדה העשרונית?"),
 answer("3.49", correct = T),
 answer("5.391848", correct = F, message = "האם ביצעתם מבחן חד זנבי?"),
 answer("5.39185", correct = F, message = "האם ביצעתם מבחן חד זנבי?"),
 answer("5.3918", correct = F, message = "האם ביצעתם מבחן חד זנבי?"),
 answer("5.392", correct = F, message = "האם ביצעתם מבחן חד זנבי?"),
 answer("5.39", correct = F, message = "האם ביצעתם מבחן חד זנבי?"),
 answer("5.4", correct = F, message = "האם ביצעתם מבחן חד זנבי?"),

 answer("11.82", correct = F, message = "שימו לב לכיוון ההשערה"),
 answer("0.29", correct = F, message = "האם זכרתם להפוך את ערך הBF כך שיתאר את יחס הסיכויים לטובת ההשערה הסבירה יותר?"),
 
 answer("2.4", correct = F, message = "שימו לב שכל שורה בטבלה מתארת את צריכת האוכל ביום מסויים ולכן המבחן המתאים יתייחס לנתונים כמזווגים"),
 answer("2.40", correct = F, message = "שימו לב שכל שורה בטבלה מתארת את צריכת האוכל ביום מסויים ולכן המבחן המתאים יתייחס לנתונים כמזווגים"),
...
נראה שהקשר בין המשתנים הוא לא מבוטל ($r_p = 0.27$) ושונה מ0 באופן מובהק סטטטיסטית ($p=0.0057$).
...
כחלק מהניתוח שלכם אתם מעוניינים להראות שאחוז הדיוק (`accuracy`) **אינו** במתאם **שלילי** עם זמן התגובה הממוצע של הנבדקים (`mean_RT`), כך שנבדקים שדייקו יותר לאו דווקא היו גם מהירים יותר.
...
שימו לב להמיר את ערך הBF שקיבלתם כך שיבטא יתרון עבור השערת האפס.
```
...
question("אילו נושאים בלומדה היו קשים במיוחד עבורך?",type="learnr_checkbox",
         ...
         answer("חשיבות ערך הסקאלה", correct=TRUE),
         answer("מבחן חד זנבי", correct=TRUE),
         answer("מדגמים בלתי תלויים", correct=TRUE),
         answer("מדגמים מזווגים", correct=TRUE),
         answer("מתאמים", correct=TRUE),
