...
שימו לב לציין את העמודה שעל פיה אנחנו מעוניינים לייצר עמודות כעמודת המפתח ("key", במקרה הזה: התפקיד במשפחה), ואת העמודה המכילה את הציונים כעמודת הערכים ("value", במקרה הזה: ציון האינטליגנציה)

```{r ex_spread, exercise=TRUE, exercise.eval = FALSE }
...
grade_this({
  ...
  target = long_df2 %>%
    tidyr::spread(key = "familial_role", value = "IQ") %>%
    dplyr::arrange(across(everything()))
  
  wrong1 = long_df2 %>%
    tidyr::spread(key = "IQ", value = "familial_role") %>%
    dplyr::arrange(across(everything()))
  
    
  wrong2 = long_df2 %>%
    tidyr::gather(key = "IQ", value = "familial_role") %>%
    dplyr::arrange(across(everything()))
  ...
  if (isTRUE(all.equal(users_df, target, check.attributes = FALSE))) {
    ...
    fail("נסו שוב. שימו לב שעמודת המפתח אמורה לכלול את התפקיד המשפחתי")
    
  }else if  (isTRUE(all.equal(wrong2, target, check.attributes = FALSE))) {
    fail("נסו שוב. כדי להמיר מפורמט ארוך לפורמט רחב יש להשתמש בפונקציה spread")
    
  }else if (ncol(users_df) == ncol(long_df2)){
    fail("נראה שמספר העמודות לא השתנה. האם העברתם את הטבלה לפורמט רחב?")
 
  }else if (!all(c("Father", "Mother") %in% names(users_df))) {
...
question_text(
  ...
  answer("0.1", correct = F, message = "שימו לב לכיוון המדד - ציון שלילי מעיד על ציון מצב רוח גבוה יותר במדידה הראשונה"),
    answer("0.10", correct = F, message = "שימו לב לכיוון המדד - ציון שלילי מעיד על ציון מצב רוח גבוה יותר במדידה הראשונה"),
  answer("1.25", correct = F, message = "שימו לב שהשאלה נוגעת להפרשים בין זמן המדידה השני והשלישי, ולא בין הראשון לשני"),
  allow_retry = TRUE,
