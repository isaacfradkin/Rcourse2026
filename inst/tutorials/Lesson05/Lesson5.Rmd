...
{
  exercises_map <- c(
  "ex_piping"     = "פקד ההעברה: תרגיל",
  "Q_select"      = "בחירת עמודות: שאלה",
  "ex_select"     = "בחירת עמודות: תרגיל",
  "ex_filter"     = "סינון שורות: תרגיל",
  "Q_filter"      = "סינון ערכים חסרים: שאלה",
  "ex_mutate"     = "עריכת נתונים: תרגיל",
  "Q_summarise_1" = "סיכום נתונים: שאלה 1",
  "Q_summarise_2" = "סיכום נתונים: שאלה 2",
  "Q_summarise_3" = "סיכום נתונים: שאלה 3",
  "ex_summary"    = "סיכום נתונים: סיכום על סמך עמודה אחת (פיצרייה)",
  "ex_summary_2"  = "סיכום נתונים: סיכום על סמך מספר עמודות",
  "ex_bind_rows"  = "צירוף נתונים: צירוף טבלאות",
  "ex_join"       = "צירוף נתונים: צירוף על סמך מספר עמודות",
  "ex_pivot_wider" = "שינוי צורה: תרגיל ראשון",
  "ex_pivot_longer"= "שינוי צורה: תרגיל שני",
  "Q_final_1"     = "תרגיל מסכם: שאלה 1",
  "Q_final_2"     = "תרגיל מסכם: שאלה 2"
)

exercises_df <- data.frame(
  exercises = names(exercises_map),
  hebrew    = as.character(exercises_map),
  row.names = NULL,
...
```{r prepare-ex_bind_rows}

df1 = data.frame(species = c("Golden Eagle", "Red Fox", "Snow Leopard"),
                 observed_count = c(2, 5, 1),
                 region = c("North Park", "North Park", "North Park"),
                 endangered = c(TRUE, FALSE, TRUE),
                 stringsAsFactors = FALSE)

df2 = data.frame(species = c("Red Fox", "Blue Iguana", "Mountain Goat"),
                  observed_count = c(3, 4, 8),
                  region = c("South Reserve", "South Reserve", "South Reserve"),
                  endangered = c(FALSE, TRUE, FALSE),
                  stringsAsFactors = FALSE)

```

```{r prepare-ex_pivot_longer}

duck_stats = data.frame(name         = c("Quack Sparrow", "Duck Norris", "Duck Vader", "Quackie Chan"),
                       agility      = c(             82,            91,           74,             88),
                       handsomeness = c(             95,            87,           79,             83),
                       IQ           = c(            110,           125,           98,            115))
...
חבילות אלו מותקנות בR כברירת מחדל, אך כדי להשתמש בהן יש לייבא אותן באמצעות שורות הקוד הבאות:
...
חלק מהפונקציות שנכיר יאפשרו לנו לבצע פעולות שכבר למדנו (כמו סינון שורות או עמודות מטבלה), אך באופן פשוט יותר, שיאפשר לנו לשמור על הקוד שלנו קריא ותמציתי. במקרים אחרים, נפגוש פונקציות המאפשרות עיבודים חדשים.
...
כדוגמה נוספת, נחשב את ממוצע שורשי הציונים.
...
# ללא פקד ההעברה
mean_sqrt = round(mean(sqrt(scores)),digits = 1)
...
```{r ex_piping,exercise=TRUE, exercise.eval = FALSE}
...
```{r ex_select, exercise=TRUE, exercise.eval = FALSE }
...
valid_young_males = need_for_cognition %>%
    filter(___________)

print(head(valid_young_males))
...
grade_this({
  ...
  user_df = user_object_get("valid_young_males") %>% arrange()
  target =  need_for_cognition %>%
    filter(gender == "male" & age < 40 & attention_check==1) %>%
    arrange()

  if (identical(user_df, target)){
    pass("מעולה!")
  }else { fail( "נסו שוב")}
...
```{r e8}
library(tidyr) # נייבא את החבילה הכוללת את הפונקציה
               # drop_na

nrow(need_for_cognition) # מספר התצפיות בטבלה המקורית


# נציין בסוגריים את שמות העמודות שנרצה לסנן על בסיסן את הנתונים
...
table2 =  need_for_cognition %>% 
  drop_na(gender, nfc_01, nfc_02r, nfc_03r)
...
כדי להחליף את הערכים החסרים נוכל להיעזר בפונקציה `replace_na`.
...
table1 = need_for_cognition %>%
  slice(c(1:1000, 36,207,6)) # נייצר עותק של 1000 השורות הראשונות של הטבלה, מלוות בחזרה נוספת על שורות 36, 207 ו6
...
שימו לב: בדוגמה הנ"ל דרסנו את הטבלה הישנה בשם need_for_cognition והחלפנו אותה בגרסה חדשה של אותה הטבלה אחרי שערכנו לה את העמודות..
כלומר - בפועל, האפקט שקיבלנו הוא של עדכון הטבלה need_for_cognition. 
...
`תרגיל`
צרו טבלה חדשה בשם need_for_cognition_2`המכילה את העמודות הבאות (בנוסף לעמודות המקוריות של הטבלה):


`nfc1_and_4`:  סכום התגובות של כל נבדק עבור שאלות 1 ו4 (היעזרו בפקד +)  
  
`attention_check`: עמודה שתחליף את העמודה הקיימת בשם זה ותכלול את המחרוזת `"Pass"` עבור מי שעבר בהצלחה את בדיקת הקשב (1) ו`"Fail"` עבור מי שנכשל בה (0)


```{r ex_mutate, exercise=TRUE, exercise.eval = FALSE}


need_for_cognition_2 =  __________
  
  
head(need_for_cognition_2)
...
grade_this({
  # שימוש בפונקציה הנכונה לשליפת האובייקט
  user_df <- user_object_get("need_for_cognition_2")
  
  # יצירת טבלת המטרה להשוואה
  target <- need_for_cognition %>% 
    mutate(
      nfc_1_and_4 = nfc_01 + nfc_04,
      attention_check = ifelse(attention_check == 0, "Fail", "Pass")
    )
  
  # 1. בדיקה ראשונית - האם העמודות בכלל קיימות?
  if (!"nfc_1_and_4" %in% names(user_df)) {
    fail("לא נמצאה עמודה בשם nfc_1_and_4. וודאו שכתבתם שם העמודה בהתאם להנחיות.")
  }
  
  if (!"attention_check" %in% names(user_df)) {
    fail("העמודה attention_check חסרה בטבלה החדשה.")
  }

  # 2. חישוב לוגי של נכונות התוכן (שימוש ב-isTRUE ו-all עם na.rm)
  # הערה: השתמשתי ב-as.character כדי לוודא שאין בעיית סוגים ב-attention_check
  check_nfc <- isTRUE(all(user_df$nfc_1_and_4 == target$nfc_1_and_4, na.rm = TRUE))
  check_att <- isTRUE(all(as.character(user_df$attention_check) == target$attention_check, na.rm = TRUE))

  # 3. מתן משוב לפי התוצאות
  if (check_nfc && check_att) {
    pass("עבודה יפה!")
  } else if (!check_nfc) {
    fail("החישוב של nfc_1_and_4 אינו תקין. האם סכמתם את nfc_01 ו-nfc_04?")
  } else if (!check_att) {
    fail("הערכים בעמודת attention_check אינם תואמים להוראות, וודאו שהמרתם אותם למחרוזת  בהתאם להנחיות")
  } else {
    fail("משהו בטבלה לא תקין, עברו על ההוראות ונסו שוב-mutate.")
  }
})
...
::: text-block1
כדי לסכם את הנתונים השתמשנו במספר פונקציות שהכרנו בעבר בהקשר של פעולות סיכום שניתן לבצע על וקטור מספרי (למשל - חישוב ממוצע). בדוגמה הנ"ל השתמשנו גם בפונקציה `n()` המשמשת לספירת כמות התצפיות בפונקציות מחבילת dplyr ובפוןנקציה is.na בשילוב עם sum כדי לחשב את כמות הערכים החסרים (ראו הערות בקוד לפירוט על האופן בו חישוב זה מתבצע)
:::
...
```{r ex_summary, exercise=TRUE, exercise.eval = FALSE, exercise.setup = "prepare-pizza"}
...
```{r ex_summary_2, exercise=TRUE, exercise.eval = FALSE, exercise.setup = "prepare-table2"}
...
בחנו את שתי הטבלאות הבאות וחברו אותם לטבלה אחת בהתאם לנתונים המוצגים בהם.
קראו לטבלה החדשה `df3`.

```{r ex_bind_rows, exercise=TRUE, exercise.eval = FALSE , exercise.setup = "prepare-ex_bind_rows"}

print(df1)
print(df2)

# df3 =  # חברו את שתי הטבלאות הנ"ל לטבלה אחת
# print(df3) # וודאו שהטבלה שקיבלתם תקינה
```


```{r ex_bind_rows-check}



grade_this({
  # שימוש בפונקציה הנכונה לשליפת האובייקט
  user_df <- user_object_get("df3")
  
  # יצירת טבלת המטרה להשוואה
  target <- bind_rows(df1,df2)
  
  wrong = bind_cols(df1,df2)
  
  # אנחנו ממיינים את שתי הטבלאות לפי כל העמודות שלהן כדי להשוות ביניהן
  user_sorted <- user_df %>% dplyr::arrange(across(everything()))
  target_sorted <- target %>% dplyr::arrange(across(everything()))
  
  # הסרת שמות שורות (row names) שעלולים להפריע להשוואה
  rownames(user_sorted) <- NULL
  rownames(target_sorted) <- NULL

  if (isTRUE(all.equal(user_sorted, target_sorted))) {
    pass("מעולה!")
  } else {
    fail("נסו שוב")
  }
...
`תרגיל:`צירוף על סמך מספר עמודות
...
```{r ex_join, exercise=TRUE, exercise.eval = FALSE, exercise.setup = "prepare-join"}
...
::: text-block1
שימו לב: במידה ולא תציינו על סמך אילו עמודות לערוך את הצירוף (באמצעות הארגומנט `by`) הצירוף יתבצע על סמך כל העמודות החופפות בין הטבלאות.
במידה והצירוף שהתכוונתם לערוך שונה מברירת המחדל הזו, צירוף כזה עשוי לייצר בעיות ולכן מומלץ לציין את העמודות שעל בסיסן תרצו לערוך את הצירוף באופן מוצהר, דרך הארגומנט `by`
:::
...
כאן יש לנו עמודה אחת המכילה את הערכים (הציון במטלה) ועמודה נוספת המציינת באיזו חזרה מדובר (ראשונה או שנייה). במידה והיינו מוסיפים נתונים מחזרות נוספות - הטבלה הייתה נהיית **ארוכה** יותר. נהוג לכנות את העמודה המפרטת את סוג המדד (בדוגמה שלנו - מספר החזרה) "עמודת המפתחות" (או בלעז: keys).
...
### pivot_wider - פורמט ארוך לפורמט רחב

הפונקציות בהן נשתמש נקראות `pivot_wider` ו-`pivot_longer` מהחבילה `tidyr`.


נתחיל עם הפונקציה `pivot_wider` שכשמה - מקבלת טבלה בפורמט ארוך ומעבירה אותה לפורמט רחב.  
בנוסף לטבלה עצמה (אותה ניתן להעביר לפונקציה באמצעות פקד העברה) הפונקציה צריכה לקבל מאיתנו שני ארגומנטים:

::: text-block1

  **1) names_from:**
שם עמודת המפתחות, המכילה את הקטגוריות שאנחנו מעוניינים להפוך לשמות עמודות. בדוגמה הקודמת קראנו לעמודה זו "iteration" והיא הכילה את הערכים "first" ו"second".
  
  **2) values_from:**
  שם עמודה המכילה את הערכים עצמם. בדוגמה הקודמת קראנו לעמודה זו "score". הערכים בעמודה זו יהוו את התוכן בעמודות החדשות שיווצרו.
...
בדוגמה הנ"ל העברנו לפונקציה טבלה בפורמט ארוך וביקשנו ממנה שתעביר אותו לפורמט רחב - כך שעמודת הציונים (score) תחולק בין עמודות שונות, המייצגות כל אחת חזרה אחרת של המטלה (בהתאם לערכים בעמודה iteration)



`תרגיל`: נסו בעצמכם

נתבונן בדוגמה נוספת. הפעם הנתונים (הפיקטיביים) מתארים את ציוני ה-IQ של בני משפחה שונים, בפורמט ארוך.
...
המירו את הטבלה לפורמט רחב.
שימו לב לציין את העמודה שעל פיה אתם מעונינים לחלק את העמודות כארגומנט `names_from` (במקרה הזה: התפקיד במשפחה), ואת העמודה המכילה את הציונים כארגומנט `values_from` (במקרה הזה: ציון האינטליגנציה).

```{r ex_pivot_wider, exercise=TRUE, exercise.eval = FALSE }

wide_df2 = long_df2 %>%  ___________

print(wide_df2)

```

```{r ex_pivot_wider-check}
grade_this({
  
  target = long_df2 %>%
    tidyr::pivot_wider(names_from = "familial_role", values_from = "IQ") %>%
    dplyr::arrange(across(everything()))
  
  wrong1 = long_df2 %>%
    tidyr::pivot_wider(names_from = "IQ", values_from = "familial_role") %>%
    dplyr::arrange(across(everything()))
  
  users_df = user_object_get("wide_df2")%>% 
    dplyr::arrange(across(everything()))
  
  
  if (isTRUE(all.equal(users_df, target, check.attributes = FALSE))) {
    pass("עבודה מצוינת!")
    
  }else if  (isTRUE(all.equal(wrong1, target, check.attributes = FALSE))) {
    fail("נסו שוב. שימו לב ששמות העמודות (names_from) אמורים להגיע מהתפקיד המשפחתי")
    
  }else if (ncol(users_df) == ncol(long_df2)){
    fail("נראה שמספר העמודות לא השתנה. האם העברתם את הטבלה לפורמט רחב?")
...
::: text-block1
מאחר והטבלה בפורמט הארוך לא הכילה ציון .IQ עבור כל תפקיד בכל אחת מהמשפחות - בטבלה בפורמט רחב ישנם ערכים חסרים (`NA`).
:::


### pivot_longer - פורמט רחב לפורמט ארוך


הפונקציה `pivot_longer` מבצעת את הפעולה ההופכית לפונקציה שראינו עכשיו. היא מקבלת טבלה בפורמט רחב ומאגדת את חלק מהעמודות שלה יחד לכדי פורמט ארוך.

הפונקציה צריכה לקבל את הפירוט הבא:

::: text-block1
1. **cols**: אילו עמודות אנחנו רוצים "לאסוף" לפורמט ארוך.

2. **names_to**: מה יהיה השם של עמודת המפתחות החדשה (זו שתכיל את שמות העמודות ש"אספנו").

3. **values_to**: מה יהיה השם של עמודת הערכים החדשה.

מאחר והארגומנטים `names_to` ו`values_to` לא מתייחסים לעמודות קיימות, אלא קובעים את שמותיהם של עמודות חדשות - נצטרך לציין את שמות העמודות כמחרוזות (כלומר - להקיף את השם הרצוי בגרשיים)
...
שורות הקוד האלו הופכות את הטבלה שיצרנו מוקדם יותר חזרה לפורמט ארוך. התוכנה תיקח את העמודות "first" ו"second", ותייצר מהן עמודה אחת בשם "iteration" המכילה את המפתחות, ועמודה נוספת בשם "score" שתכיל את הערכים עצמם.


`תרגיל`: פורמט רחב לארוך

בטבלה `duck_stats` מפורטים ציוניהם של ארבעה ברווזים בשלושה מדדים שונים (זריזות, יופי ואינטליגנציה).
המירו את הטבלה לפורמט ארוך כך שהיא תכלול שלוש עמודות: שם הברווז, שם המדד הרלוונטי והציון של הברווז באותו מדד.

את הטבלה החדשה שמרו בשם `long_duck_stats`.


```{r ex_pivot_longer, exercise=TRUE, exercise.eval = FALSE, exercise.setup = "prepare-ex_pivot_longer"}

print(duck_stats)

#long_duck_stats = 

# print(long_duck_stats)

```

```{r ex_pivot_longer-check}
grade_this({
  
  target <- duck_stats %>% 
    tidyr::pivot_longer(cols = c(agility, handsomeness, IQ),
                        names_to = "measure",
                        values_to = "score") %>% 
    arrange(across(everything())) 


  user_df <- user_object_get("long_duck_stats") %>% 
    arrange(across(everything()))

  # Correct answer (ignoring exact col names if they chose something else, though target uses measure/score)
  if (nrow(user_df) == 12 && ncol(user_df) == 3 && !("name" %in% user_df[[2]])) {
    pass("עבודה מצוינת!")
    
  # All columns gathered, including name
  } else if (nrow(user_df) > 12) {
    fail("נסו שוב. האם זכרתם להשאיר את עמודת השמות מחוץ לקיבוץ העמודות (באמצעות cols)?")
    
  # Only two of the three measures gathered
  } else if (nrow(user_df) < 12) {
    fail("נסו שוב. האם זכרתם לקבץ את כל שלוש עמודות המדדים?")
...
היחידה הזו ארוכה, ומכילה הרבה חומר חדש. 
בהתאם, התרגיל המסכם דורש יישום של כמה נושאים חדשים ויתכן שתצטרכו לחזור לחלקים הרלוונטים בלומדה או בסיכומים שלכם כדי לפתור אותו.
בהצלחה!
...
הורידו מהמודל את הקובץ `mood.csv` וטענו אותו לסביבת העבודה שלכם בR.
...
לאחר שהעברתם את הטבלה לפורמט רחב - תוכלו לחשב שתי עמודות חדשות, אחת עבור כל אחד מההפרשים.

אחרי שחישבתם את הפרשי דירוגי מצב הרוח, צרו טבלה חדשה המכילה רק את מספר הנבדק ואת שתי עמודות ההפרשים שחישבתם. המירו את טבלה זו לפורמט ארוך, כך שבטבלה החדשה יהיו לכם שני ציוני הפרשים עבור כל נבדק, המיוצגים בשתי שורות נפרדות, ועמודת מפתחות שמציינת לגבי כל שורה לאיזה מההפרשים היא מתייחסת (בין T1 לT2 או בין T2 לT3).

סכמו את הנתונים באמצעות `group_by` ו `summarise` כך שתקבלו את הערך הממוצע וסטיית התקן של כל אחד מההפרשים.

אתם יכלים להיעזר בתבנית הבאה על מנת לפתור את התרגיל. יש לפתוח קובץ סקריפט בRstudio, להעתיק אליו את התבנית ולהשלים את החלקים החסרים.
...
print(mood_diff_summary)
```


```{r Q_final_1, echo = FALSE}
question_text(
  "<div dir='rtl' style='text-align: right;'>
  מה היה ההפרש הממוצע בציוני מצב רוח בין זמן המדידה <strong>
  הראשון</strong>
  ל<strong>שני</strong>?<br>
  <span style='font-weight: normal;'>
  עגלו לשני מקומות לאחר הנקודה העשרונית.<br>
  ציון חיובי מעיד על מצב רוח מרומם יותר בנקודת המדידה השנייה.
  </span>
  </div>",
  answer("1.25", correct = TRUE),
  answer("-1.25", correct = F, message = "שימו לב לכיוון המדד - ציון שלילי מעיד על ציון מצב רוח גבוה יותר במדידה הראשונה"),
  answer("-0.1", correct = F, message = "שימו לב שהשאלה נוגעת להפרשים בין זמן המדידה הראשון והשני, ולא בין השני והשלישי"),
  allow_retry = TRUE,
...
```{r Q_final_2, echo = FALSE}
question_text(
  "<div dir='rtl' style='text-align: right;'>
  מה הייתה סטיית התקן של ההפרשים בציוני מצב רוח בין זמן המדידה <strong>
  השני</strong>
  ל<strong>שלישי</strong>?<br>
  <span style='font-weight: normal;'>
  עגלו לשני מקומות לאחר הנקודה העשרונית.<br>
  ציון חיובי מעיד על מצב רוח מרומם יותר בנקודת המדידה השלישית.
  </span>
  </div>",
  answer("2.02", correct = TRUE),
  answer("2.24", correct = F, message = "שימו לב - השאלה נוגעת להפרשי הדירוגים בין זמן המדידה השני לשלישי"),
  allow_retry = TRUE,
...
```{r survey_q_tirgul, echo = FALSE}
question("האם השתתפת בתרגול שהוקדש ליחידה זו?",
         type="single",
         answer("כן"        ,correct=TRUE,  message = "תגובתך: כן"),
         answer("לא"    ,correct=TRUE,  message = "תגובתך: לא"),
         answer("התרגול עוד לא התקיים"     ,correct=TRUE,  message = "תגובתך: התרגול עוד לא התקיים"),
         answer("לא,אבל צפיתי בהקלטה"        ,correct=TRUE,  message = "תגובתך: לא,אבל צפיתי בהקלטה"),
         correct = "",
         incorrect = "",
         allow_retry = TRUE
)
