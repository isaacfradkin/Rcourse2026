...
##### פונקציה לבדיקה
exercises_df = data.frame(exercises = c("cor_ex", "cor_mat_ex", "corr_table_question", "cor_mat_q", "cor_test_q", "cor_test_ex", "final_ex_1", "final_ex_2", "final_ex_3"), 
  hebrew = c("תרגיל - קורלציה"
             , "תרגיל - טבלת קורלציות"
             , "1 שאלה - טבלת קורלציה"
             , "2 שאלה - טבלת קורלציה"
             , "שאלה - מובהקות קורלציה"
...
היום אנחנו נלמד כיצד לבחון מגמות לינאריות בנתונים שלנו. תזכורת, מגמה לינארית מתארת מצב בו הקשר בין שני משתנים (למשל רמת העושר והכנסה) יכול להיות מתוארת על ידי קו ישר (למשל, ככל שההכנסה גבוהה יותר כך גם רמת העושר). 
...
מתאם (למשל מתאם פירסון)  
מבחן רגרסיה (ניבוי של משתנה אחד ממשתנים אחרים)   
נלמד היום איך לבצע את שני סוגי המבחנים בR ואיך לפרש מתוצאותיהם עד להבין עד עד כמה מתקיים קשר לינארי בין שני משתנים והאם קשר זה מובהק.
...
הנתונים הללו נלקחו ממדגם אמיתי
...
### טבלת קורלציה
הפונקציה cor מספקת לנו גם דרך נוחה לחשב מסה של קורלציות יחדיו. 
...
היינו יכולים לבדוק תכונה תכונה, אבל זה יקח הרבה זמן, אז פשוט נחשב את הקורלציות של כל שאלות השאלון יחד. 
...
לטובת איור זה נשתמש בחבילה חדשה, שגם שייכת לקבוצת החבילות של ggplot.  
...
### בדיקת מובהקות
אז עד כה חישבנו את הקורלציות, אבל איך אנחנו יכולים לדעת האם הקורלציה משמעותית מספיק? 
...
לכן – אם אנחנו רוצים להסיק מסקנות על האוכלוסייה נצטרך לבחון האם התוצאות שלנו מובהקות סטטיסטית, כמה סביר היה לקבל מתאם כמו שקיבלנו בטעות (כאשר אין מתאם באוכלוסיה).  
ייתכן ואתם עוד לא מבינים את הרעיון הזה עד הסוף, וזה בסדר. מובהקות גם בהקשר זה מתבבסת על תהליך הדומה לזה הנעשה במבחן t. יכול להיות שבהמשך הסמסטר יהיה לכם יותר קל להבין את המושגים.  
...
נסו לקרוא את הפלט לב ולענות על השאלות הבאות:
...
על ידי שימוש בsummary() אנחנו מקבלים מידע נוסף על המודל שהרצנו, על.
הפלט מחולק לכמה חלקים:  
`Call`
תיאור המודל שהרצנו.  

`Residuals`  
השיירים (Residuals) אשר מספקים לנו אומדן על הסטייה הממוצעת של התצפיות מהקו הישר, ופירוט נוסף על הקבועים שחושבו עבור המודל. 
...
`תרגיל 1`  
קודם כל בואו נבחן את הנתונים מ2010 במבט קרוב יותר. צרו שני מודלים לינארים לנתונים, אחד עבור התצפיות שרמת ההכנסה שלהם היא עד 90 אלף דולר (כולל) בשנה ואחד עבור אלה שמרוויחים יותר.  
